...
public partial class Conversation : ContentPage
{
    ...
    private void AddMusicShare(string songTitle, string artistName, string time, bool isOutgoing = true, bool isRead = true)
    {
        var messageLayout = new HorizontalStackLayout
        {
        ...
        var messageBorder = new Border
        {
            BackgroundColor = isOutgoing ? Color.FromArgb("#1db954") : Color.FromArgb("#212121"),
        ...
        {
            BackgroundColor = Color.FromArgb("#191414"),
            StrokeShape = new RoundRectangle { CornerRadius = new CornerRadius(8) },
            HeightRequest = 50,
            WidthRequest = 50
        };

        var musicIcon = new Label
        {
            Text = "♪",
            FontSize = 24,
            TextColor = Colors.White,
        ...
        albumArt.Content = musicIcon;
        musicGrid.Add(albumArt, 0, 0);
        ...
        var artistLabel = new Label
        {
            Text = artistName,
            FontSize = 12,
            TextColor = isOutgoing ? Color.FromArgb("#f0f0f0") : Color.FromArgb("#b3b3b3"),
            LineBreakMode = LineBreakMode.TailTruncation
        };

        infoStack.Add(titleLabel);
        infoStack.Add(artistLabel);
        musicGrid.Add(infoStack, 1, 0);

        contentStack.Add(musicGrid);
        ...
        var timeLabel = new Label
        {
            Text = time,
            FontSize = 10,
            TextColor = isOutgoing ? Color.FromArgb("#f0f0f0") : Color.FromArgb("#888888")
        };
    ...
    private async void OnShareMusicClicked(object sender, EventArgs e)
    {
        try
        {
            // TODO: Implement music picker/search
            // For now, show a simple example
            var currentTime = DateTime.Now.ToString("h:mm tt");
            AddMusicShare("Anti-Hero", "Taylor Swift", currentTime, true, false);

            await DisplayAlert("Share Music", "This feature will let you search and share songs from Spotify", "OK");
        }
        catch (Exception ex)
        {
            Debug.WriteLine($"Error sharing music: {ex.Message}");
        }
    }
